...
const char *version = "\0$VER: touch 1.2 (19-Jan-2018) © Chris Hooper";
...
datec(const char *str)
{
    ...
    pos = 0;
    count = sscanf(substr, "%2d%n%2d%n%2d%n%2d%n.%n%2d%n", &month, &pos, &day, &pos, &hour, &pos, &minute, &pos, &pos, &second, &pos);
