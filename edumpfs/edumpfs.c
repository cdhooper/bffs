...
const char *version = "\0$VER: edumpfs 1.5 (08-Feb-2018) © Chris Hooper";
...
print_superblock(int partition, ULONG offset)
{
    ...
    printf("sbsize=%-6d altsb=%d\n",
           ...
           cgsblock(superblock, disk32(superblock->fs_ncg) - 1));
...
{
    if (active[partition] && read_superblock(partition, sb_offset[partition])) {
        if (show_superblock)
...
print_sun_disk_label(void)
{
    ...
    label = (sun_label_t *) AllocMem(DEV_BSIZE, MEMF_PUBLIC | MEMF_CLEAR);
    ...
    printf("%s\n", label->bb_mfg_label);
    printf("    Cylinders=%-5d Reserved=%-2d Available=%-5d "
           "Media Interleave=%d\n",
...
print_bsd44_disk_label(void)
{
    ...
    printf("       Cylinders=%-4d    Cyl Reserved=%-3d          "
           "Spares/Track=%d\n",
    ...
    printf("   Sectors/Track=%-3d Sectors/Cylinder=%-5d         "
           "Sector Size=%d\n",
    ...
    printf("\n Part  StartBlk  Num_Blks     #_MB FSize BSize      "
           "Start        End\n");
    for (pnum = 0; pnum < MAX_PART; pnum++) {
        if (disk32(label->bb_part[pnum].fs_size) != 0) {
            ...
            printf("%2d(%c) %9d %9d ", pnum, pnum + 'a', fs_offset, fs_size);
...
print_help(void)
{
    fprintf(stderr,
        ...
        "label information.  It does this by directly reading the media of "
        "the\n"
        ...
        if (*ptr == '-') {
            for (ptr++; *ptr != '\0'; ptr++) {
                if (*ptr == 'b') {
                    ...
                    if (index >= argc) {
                        fprintf(stderr,
                                "-%c option requires 512-sector number\n",
                                *ptr);
                        print_usage();
                    }
                    ...
                    if (index >= argc) {
                        fprintf(stderr,
                                "-%c option requires block number\n", *ptr);
                        print_usage();
                    }
