...
const char *version = "\0$VER: dcp 1.3 (08-Feb-2018) © Chris Hooper";
...
print_help(void)
{
    ...
    fprintf(stderr,
        "%s is a program which will does block copies from the specified"
        " source\n"
            ...
            if (!isalpha(*(pos + 1))) {
                ...
                if (pos2)
                    if (!isalpha(*(pos2 + 1))) {
                        ...
                        fprintf(stderr, "Error opening device %s: for %s; "
                                "flags=%s; flags must be an integer!\n",
                                fdes->name, (dtype == dREAD) ? "read" : "write",
                                pos2 + 1);
                        open_failure++;
                        goto failed_open;
                ...
                fprintf(stderr, "Error opening device %s for %s; you must "
