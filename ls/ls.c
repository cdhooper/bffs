...
const char *version = "\0$VER: ls 1.2 (08-Feb-2018) © Chris Hooper";
...
int isfirst = 1;
int printed = 0;
int filesize_digits = 1;
ULONG filesize_max = 0;
          ...
          const char *name)
{
    ...
    if (lflag || gflag) {
        ...
        if (((pchar == 'b') || (pchar == 'c')) && (fattr != NULL)) {
            ...
            printf("%*s ", hflag ? 4 : filesize_digits, buf);
        } else if (hflag) {
            ULONG size = fib->fib_Size;
            if (size < 1000) {
                printf("%5u ", size);
            } else {
                const char sizes[] = "KMGTPEZY";
                int pos = 0;
                for (pos = 0; size >= 1900 * 100; pos++)
                    size >>= 10;
                size += 52;
                if (size >= 1024 * 10)
                    printf("%4u", size >> 10);
                else
                    printf("%2u.%u", size >> 10, ((size * 10) >> 10) % 10);
                printf("%c ", sizes[pos]);
            }
        } else {
            printf("%*ld ", filesize_digits, fib->fib_Size);
        }
...
*/
    ...
    for (current = sortlist; current != NULL; current = current->next) {
        if (tflag) {
            ...
            temp = strcompare(current->name, node->name) * (rflag ? -1 : 1);
            if (temp >= 0)
                break;
        }
        parent = current;
...
*/
    ...
    if (filesize_max < fib->fib_Size) {
        ULONG tempsize = fib->fib_Size;
        filesize_max = fib->fib_Size;
        for (filesize_digits = 0; tempsize > 0; tempsize /= 10)
            filesize_digits++;
    }
...
{
    fprintf(stderr, "%s\n"
            "usage: %s [-aAcdfFghHilnprRsStu1] [<filename...>]\n"
